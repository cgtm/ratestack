...
function hasCompleteRatesForBase(base) {
  ...
  return store.selected.every(
    (c) => c === base || store.rates[base][c] !== undefined,
  );
}
...
export async function fetchRates() {
  ...
  try {
    ...
    store.selected.forEach((c) => {
      if (c !== base && data.rates[c] !== undefined)
        relevant[c] = data.rates[c];
    });
...
export function recalculate() {
  ...
  store.selected.forEach((code) => {
    ...
    if (
      store.rates[store.baseCurrency] &&
      store.rates[store.baseCurrency][code]
    ) {
  ...
  store.selected.forEach((code) => {
    ...
    const rateText =
      code === store.baseCurrency
        ? ""
        : getRateDisplay(store.baseCurrency, code);
